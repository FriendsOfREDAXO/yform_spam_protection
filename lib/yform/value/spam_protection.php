...
class rex_yform_value_spam_protection extends rex_yform_value_abstract
{
    public function postValidateAction(): void
    {
        ...
        if ((null !== rex_backend_login::createUser()) && ('1' === rex_config::get('yform_spam_protection', 'ignore_user'))) {
                ...
                if (1 == $this->params['send']) {
                    ...
                    if (trim((string) rex_config::get('yform_spam_protection', 'use_stopforumspam'), '|')) {
                        ...
                        if ($result) {
                            $doc = new DOMDocument();
                            $doc->loadXML($result);
                            if ('yes' == $doc->getElementsByTagName('appears')[0]->nodeValue) {
                                $this->params['warning'][$this->getId()] = $this->params['error_class'];
